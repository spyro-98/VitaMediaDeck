...
description: How the three sibling decoder and HTTPS repositories build together and are selected at runtime.
icon: Boxes
---
...
3. if `h264_vita` fails later while receiving the first access units, close the
   hardware session and reopen software near the last media timestamp;
...
libcurl/OpenSSL archives are not an accepted fallback.
`tools/build-libssh2-vita.sh` also builds libssh2 with Mbed TLS so the final
executable contains no legacy OpenSSL symbols.