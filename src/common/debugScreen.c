...
int psvDebugScreenInit() {
	...
	if (displayblock < 0) {
		/* The mutex was already created above. Without this cleanup, each
		 * failed attempt (debug_screen_init_retry() in main.c can make five
		 * consecutively) leaks one SceUID mutex permanently. Found during the
		 * extended multi-agent review on 2026-08-04 and confirmed by two
...
int psvDebugScreenPuts(const char * _text) {
	...
	/* Once the debug screen is finished, `base` points to released CDRAM and
	 * `mutex` names a deleted UID. Late diagnostics must be a silent no-op
	 * instead of writing through freed memory or blocking on a dead mutex. */
	if (!initialized) {
		return 0;
	}
